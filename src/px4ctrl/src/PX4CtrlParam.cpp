...
{
	read_essential_param(nh, "mode", mode);
	read_essential_param(nh, "policy_modality", policy_modality);

	if (mode == "neural")
	{
		read_essential_param(nh, "model_path", model_path);
		
		read_essential_param(nh, "post_hover_mode", post_hover_mode);
		read_essential_param(nh, "switch_hover_x", switch_hover_x);
		read_essential_param(nh, "post_hover_pos/x", post_hover_pos[0]);
		read_essential_param(nh, "post_hover_pos/y", post_hover_pos[1]);
		read_essential_param(nh, "post_hover_pos/z", post_hover_pos[2]);
	}
	...
	read_essential_param(nh, "msg_timeout/hil_cmd", msg_timeout.hil_cmd);

	read_essential_param(nh, "pose_solver", pose_solver);
	read_essential_param(nh, "mass", mass);
	read_essential_param(nh, "gra", gra);
	
	if (read_optional_param(nh, "ctrl_freq_max", ctrl_freq_max))
	{
		ROS_WARN("[px4ctrl] Using default spin fsm mode.");
		spin_mode = 0;
	}
	else if (read_optional_param(nh, "ctrl_interval", control_interval) && read_optional_param(nh, "spin_interval", spin_interval))
	{
		ROS_WARN("[px4ctrl] Using high-frequency fsm mode.");
		spin_mode = 1;
	}
	else
	{
		ROS_ERROR("[px4ctrl] Please specify either \"ctrl_freq_max\" or \"ctrl_interval\" and \"spin_interval\".");
		ROS_ERROR("[px4ctrl] Shutting down the node.");
		ros::shutdown();
	}
	...
	read_essential_param(nh, "thrust_model/accurate_thrust_scale", thr_map.accurate_thrust_scale);
	ROS_WARN("accurate_thrust_model is %d  accurate_thrust_scale is %f ",thr_map.accurate_thrust_model,thr_map.accurate_thrust_scale);

	//kdkd localtime print info
	// read_essential_param(nh, "drone_id", drone_id);
	// read_essential_param(nh, "formation_num", formation_num);
	//kdkd collision threshould	
