	...
	case CMD_CTRL:
	{
		...
		else if (param.mode == "neural")
		{
			...
			else if (!rc_data.is_command_mode)
			{
				...
				set_hov_with_odom();
				des = get_hover_des();
				ROS_INFO("\033[32m[px4ctrl] From CMD_CTRL(L3) to AUTO_HOVER(L2)!\033[32m");
			}
			else  // run the policy
			{
				...
				{  
					if (odom_data.p(0) > param.switch_hover_x) 
					{
						...
						des = get_hover_des();
						ROS_INFO("\033[32m[px4ctrl_nnpolicy] From CMD_CTRL(L3) to AUTO_HOVER(L2) due to cross!!!\033[32m");
					}
				}
