...
#include "nnpolicy.h"
//kdkd px4state to drone
// #include <traj_utils/to_drone_state.h>
...
public:
	...
	Controller_Output_t ctrl_FCU_data;  // especially for HIL experiments
	Battery_Data_t bat_data;
	Takeoff_Land_Data_t takeoff_land_data;
	ros::Time latest_hil_cmd_time;
	Switch2Hover if_switch2hover;
	//kdkd localtime date
	// local_time_Date_t local_time_data;
	
	Controller &controller;
	NNPolicy &policy;
	bool is_nn_warmup = false;
	bool is_unsafe_time = false;
	...
	//kdkd localtime pub
	// ros::Publisher local_time_pub;
	
	// //kdkd px4state to drone state
	// ros::Publisher px4_state_to_drone_node_pub;
	// traj_utils::to_drone_state px4_node_state;
	...
	enum State_t
	{
		...
		ATTI_RECOVER, // px4ctrl is actived, and controling the drone to recover the attitude.
		AUTO_TAKEOFF,
		AUTO_LAND
	};
	...
	PX4CtrlFSM(Parameter_t &, Controller &, NNPolicy &);
	void process();
	...
	bool hil_cmd_is_received(const ros::Time &now_time);
	bool recv_new_odom();
...
private:
	...
	void set_hov_with_odom_for_cross();

	void set_hov_with_rc();

	// ---- nn inference ----
	void check_inference(const Odom_Data_t &odom);
