...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    check_inference_mode = ((double)msg.channels[9] - 1000.0) / 1000.0; // 10通
    ...
    if (!have_init_last_check_inference_mode)
    {
        have_init_last_check_inference_mode = true;
        last_check_inference_mode = check_inference_mode;
    }
    ...
    if (is_hover_mode)
    {
        ...
        if (last_check_inference_mode < GEAR_SHIFT_VALUE && check_inference_mode > GEAR_SHIFT_VALUE)
        {
            is_check_inference_mode = true;
            ROS_INFO("\033[32m[px4ctrl] Enable policy!!!\033[32m");
        }
        else if (check_inference_mode < GEAR_SHIFT_VALUE)
            is_check_inference_mode = false;
    }
...
void Controller_Output_t::feed(mavros_msgs::AttitudeTargetConstPtr pMsg)
{
    // only support bodyrate control mode
    bodyrates.x() = pMsg->body_rate.x;
    bodyrates.y() = pMsg->body_rate.y;
    bodyrates.z() = pMsg->body_rate.z;
...
void Battery_Data_t::feed(sensor_msgs::BatteryStateConstPtr pMsg)
{
    ...
    volt = 0.3 * volt + 0.7 * voltage; // Naive LPF, cell_voltage has a higher frequency
...
Switch2Hover::Switch2Hover()
{
    rcv_stamp = ros::Time(0);
}

void Switch2Hover::feed(const std_msgs::Empty::ConstPtr& pMsg)
{
    rcv_stamp = ros::Time::now();
    triggered = true;
    ROS_INFO("setting switch2hover triggered to true");
}

void Switch2Hover::feed_false(const std_msgs::Empty::ConstPtr& pMsg)
{
    rcv_stamp = ros::Time::now();
    triggered = false;
    ROS_INFO("setting switch2hover triggered to false");
}
// //kdkd localtime
// local_time_Date_t::local_time_Date_t()
// {

// }
// //kdkd localtime
// void local_time_Date_t::feed(traj_utils::LocalTimeConstPtr pMsg)
// {
//     msg = *pMsg;
//     rcv_stamp = ros::Time::now();
//     static ros::Time t_start = ros::Time::now();
//     rcv_stamp = ros::Time::now();
//     if((rcv_stamp - t_start).toSec() < 1)
//     {
//         return;
//     }
//     if (abs((rcv_stamp - pMsg->start_time).toSec()) > 10.0)
//     {
//         not_synchronized = true;
//         ROS_WARN("[px4ctrl] Please wait 1min to start swarm after ntp synchronied or you need ntp syc!!! Time stamp diff: Local - Remote Agent %d = %fs, swarm time seems not synchronized, abandon!",
//                      pMsg->drone_id, (rcv_stamp - pMsg->start_time).toSec());
//     }
//     // if (pMsg->no_syc)
//     // {
//     //     ROS_WARN("Time not syc,can not takeoff!!");
//     //     not_synchronized = true;
//     // }
//     // for (size_t i = 0; i < formation_nums; i++)
//     // {
//     //     if (pMsg->drone_id == i)
//     //     {
//     //         if (pMsg->drone_id == my_id)
//     //         {
//     //             continue;
//     //         }else
//     //         {
//     //             if (abs((rcv_stamp - pMsg->start_time).toSec()) < 0.25)
//     //             {
//     //                 std::cout<<"rcv_stamp:"<<rcv_stamp<<std::endl;
//     //                 std::cout<<"pMsg->start_time:"<<pMsg->start_time<<std::endl;
