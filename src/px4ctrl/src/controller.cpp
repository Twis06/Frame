...
quadrotor_msgs::Px4ctrlDebug Controller::update_alg0(
    ...
    double voltage)
{
  // ROS_INFO("in update_alg0 case   des = %f %f %f  odom = %f %f %f",des.p(0),des.p(1),des.p(2),odom.p(0),odom.p(1),odom.p(2));
...
quadrotor_msgs::Px4ctrlDebug Controller::attitude_recovery_control(
    const Imu_Data_t &imu,
    Controller_Output_t &u,
    double voltage)
{
    ...
    u.bodyrates = computeFeedBackControlBodyrates(desired_q, imu.q, param);

    // reactive thrust control: solve the problem min(6<=u<=14){-alpha*|T_v - g|-|T_h|}
    ...
    double T_z_balance = std::max(9.0, std::min(14.0, 10.5 / cos_theta));
    double T_v_balance = T_z_balance * cos_theta;
    double T_h_balance = T_z_balance * sin_theta;

    double alpha = 2.0; // object weight
    double object_T_max = -alpha * abs(T_v_max - 10.5) - abs(T_h_max);
    double object_T_min = -alpha * abs(T_v_min - 10.5) - abs(T_h_min);
    double object_T_balance = -alpha * abs(T_v_balance - 10.5) - abs(T_h_balance);
    ...
    double voltage)
{
    ...
    if (param.thr_map.accurate_thrust_model)
    {
        normalized_thrust = 1.06 * AccurateThrustAccMapping(des_acc_norm, voltage, param);
...
bool Controller::estimateThrustModel(
    ...
    const Parameter_t &param)
{
  timed_thrust.push(std::pair<ros::Time, double>(ros::Time::now(), 0.21));
  while (timed_thrust.size() > 100)
    timed_thrust.pop();
