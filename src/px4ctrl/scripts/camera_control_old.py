...
class CamCtrlNode:
    ...
    def setup_camera(self):
        ...
        ret=self.cam.MV_CC_FeatureSave("/home/nv/px4_policy_deploy_plus/src/px4ctrl/scripts/camera_saved_config")
        if ret != 0:
            print ("\033[31m Set camera config fail! ret[0x%x]\033[0m" % ret)
            sys.exit()  
        ...
        while not self.g_bExit: 
            ...
            if ret == 0:
                ...
                if not self.if_rgb:
                    ...
                    img = self.calculate_mask_new(img_raw)
    ...
    def calculate_mask(self, img_raw) -> np.array:
    ...
    def calculate_mask_new(self, img_raw) -> np.array:     

        hsv_ranges = [
                (np.array([0, 130, 10]), np.array([7, 180, 30])),     
                (np.array([170,130,10]),np.array([180,180,30])),
    ...
    def update_gap_id(self, img_raw: np.array, mask) -> np.array:
        if self.detect_gap_id < self.num_gap - 1:
            if np.sum(mask) < 8:
                ...
                return self.calculate_mask_new(img_raw)
            else:
                return mask
        else:
            return mask
        ...
        while not self.g_bExit:
            ...
            if img is not None:
                ...
                if self.auto_stop:
                    ...
                    #     self.stop_counter += 1
                    if np.sum(img) < 15 and self.warmup_done:
                        if not self.stop_counter >= 9:
                            self.stop_counter += 1
    ...
    def pixel_based_control(self):
        if (self.p[0] < self.last_gap_pos[0] + 0.2 and not self.auto_stop) or (self.auto_stop and self.stop_counter < 8): # (self.auto_stop and self.stop_counter < 9):
        ...
        elif self.auto_stop and self.stop_counter >= 8 and self.warmup_done and self.receive_rc_cmd:
