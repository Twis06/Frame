...
def work_thread(cam=0, pData=0, nDataSize=0):
    ...
    while True:
        ...
        if ret == 0:
            ...
            cv2.imwrite('img_origin_%d.jpg' % stOutFrame.stFrameInfo.nFrameNum, img)
            ...
            # print ("Image Arrive Time[%f]" % arrived_time)

            #cv2.imwrite('img_%d.jpg' % stOutFrame.stFrameInfo.nFrameNum, img)
            # cv2.waitKey(1)

            elapsed_time = time.time() - time_trigger
            print ("Image Arrive Time[%f]" % elapsed_time)
            sleep_time = max(0, interval - elapsed_time)  # 确保不会出现负值
