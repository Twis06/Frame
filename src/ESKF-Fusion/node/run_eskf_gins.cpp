...
int main(int argc, char** argv) {
  ros::init(argc, argv, "run_gins_node");
  ...
  // 初始化器
  estimator::StaticIMUInit imu_init;  // 使用默认配置
  estimator::ESKFD eskf;

  common::TxtIO io(FLAGS_txt_path);
  ...
  auto save_result = [&save_vec3, &save_quat](
                         std::ofstream& fout,
                         const common::NavStated& save_state) {
  ...
  io.SetIMUProcessFunc([&](const common::IMU& imu) {
      /// IMU 处理函数
      ...
      .SetGNSSProcessFunc([&](const common::GNSS& gnss) {
        /// GNSS 处理函数
        if (!imu_inited) {
          return;
        }

        common::GNSS gnss_convert = gnss;
        if (!common::ConvertGps2UTM(gnss_convert, antenna_pos,
                                    FLAGS_antenna_angle) ||
      ...
      .SetOdomProcessFunc([&](const common::Odom& odom) {
