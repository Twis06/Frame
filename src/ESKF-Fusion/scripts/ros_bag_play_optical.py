...
def main(bag_file, optical_flow_topic, imu_topic, skip_time_second):
    rospy.init_node("rosbag_player")
    imu_pub = rospy.Publisher(imu_topic, Imu, queue_size=100)
    optical_flow_pub = rospy.Publisher(optical_flow_topic, OpticalFlowRad, queue_size=100)
...
if __name__ == "__main__":
    ...
    if not check_topic_in_bag(
        args.bag_file, args.optical_flow_topic
    ) or not check_topic_in_bag(args.bag_file, args.imu_topic):
        exit(0)
    
    main(args.bag_file, args.optical_flow_topic, args.imu_topic, args.skip_start_second)
