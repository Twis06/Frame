...
#include <estimator/static_imu_init.h>
#include <glog/logging.h>

namespace estimator {

bool StaticIMUInit::AddIMU(const common::IMU& imu) {
  if (init_success_) {
    return true;
  }
...
bool StaticIMUInit::AddOdom(const common::Odom& odom) {
  // 判断车辆是否静止
  if (init_success_) {
    return true;
  }
...
bool StaticIMUInit::TryInit() {
  ...
  common::math::ComputeMeanAndCovDiag(init_imu_deque_, mean_gyro, cov_gyro_,
                              [](const common::IMU& imu) { return imu.gyro_; });
  common::math::ComputeMeanAndCovDiag(init_imu_deque_, mean_acce, cov_acce_,
                              [this](const common::IMU& imu) { return imu.acce_; });
  ...
  // 重新计算加计的协方差
  common::math::ComputeMeanAndCovDiag(
      init_imu_deque_, mean_acce, cov_acce_,
      [this](const common::IMU& imu) { return imu.acce_ + gravity_; });
